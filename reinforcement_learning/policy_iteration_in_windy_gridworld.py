'''
An implementation of the policy iteration algorithm for
the Gridworld game (see 'gridworld.py' for reference).

We now assume that the state-transition probabilities, p(s',r|s,a),
are random, s.t. p(a|s) = P_A (the global variable in the script below),
and p(!a|s) = (1 - P_A) / len(all_!a).
...
THRESHOLD = 1e-4 # to check the convergence of the value function
GAMMA = 0.9 # the discount factor
ALL_POSSIBLE_ACTIONS = ['U', 'D', 'L', 'R']
P_A = 0.5 # p(a|s) - the probability of moving in the desired direction
				...
				if s in grid.actions:
					...
					for a in ALL_POSSIBLE_ACTIONS:
						...
						if a == policy[s]:
							...
							p_s_prime_and_r = (1 - P_A) / (len(ALL_POSSIBLE_ACTIONS) - 1) 
