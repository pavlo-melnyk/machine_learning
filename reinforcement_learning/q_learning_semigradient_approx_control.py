'''
An implementation of the Q-Learning Semi-Gradient algorithm
...
def best_value_and_action(s):
	...
	and the corresponding action (and the features
...
if __name__ == '__main__':
	...
	# for s in grid.actions.keys():
	# 	for a in ALL_POSSIBLE_ACTIONS:
	# 		print(s, a, '--->', feature_transformer(s, a))
	# exit()
	...
	for i in range(N_EPISODES):
		...
		while not grid.game_over:
			...
			# cur_a = np.random.choice(ALL_POSSIBLE_ACTIONS) # an episode will run slower
			cur_x = feature_transformer(cur_s, cur_a) # the gradient in the update equation
			...
			if grid.is_terminal(s):
				...
				# we also need to know the best of a(t+1):
				max_q_hat, a = best_value_and_action(s)
				a = random_action(a, eps=0.5/t)
				target = r + GAMMA*max_q_hat			
